document.addEventListener('DOMContentLoaded', () => {
    ...
    const dom = {
        ...
        itemQuantity: document.getElementById('itemQuantity'), itemUnit: document.getElementById('itemUnit'), // Novo campo
    ...
    function renderTotals(list) {
        ...
        list.forEach(i => {
            const price = parseFloat(i.Preco) || 0;
            const quantity = parseFloat(i.Quantidade) || 1;
            if (i.Status === 'No Carrinho') cartTotal += (price * quantity);
    ...
    function initializeAppListeners() { if (appListenersInitialized) return; dom.checkoutBtn.addEventListener('click', handleCheckout); dom.archiveBtn.addEventListener('click', handleArchive); dom.archiveFilter.addEventListener('change', renderLists); dom.showPurchasedToggle.addEventListener('change', renderLists); dom.categoryFilter.addEventListener('change', renderLists); dom.pendingList.addEventListener('click', handleListClick); dom.purchasedList.addEventListener('click', handleListClick); dom.addItemBtn.addEventListener('click', () => { dom.addItemForm.reset(); dom.itemQuantity.value = 1; dom.modal.style.display = 'block'; }); dom.closeBtn.addEventListener('click', () => dom.modal.style.display = 'none'); window.addEventListener('click', (e) => { if (e.target == dom.modal) dom.modal.style.display = "none"; }); dom.addItemForm.addEventListener('submit', handleFormSubmit); appListenersInitialized = true; }

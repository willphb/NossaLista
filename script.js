document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderLists() {
        ...
        // CORREÇÃO 2: A lógica do filtro de categoria estava errada.
        ...
        listToRender.forEach(item => {
            ...
            if (['Pendente', 'No Carrinho', 'Arquivado'].includes(item.Status)) { dom.pendingList.appendChild(itemCard); hasPendingOrInCart = true; }
            else if (item.Status === 'Comprado') { dom.purchasedList.appendChild(itemCard); hasPurchased = true; }
        });
    ...
    function populateArchiveFilter() {
        ...
        [...months.entries()].sort().reverse().forEach(([key, value]) => { const cap = value.charAt(0).toUpperCase() + value.slice(1); dom.archiveFilter.innerHTML += `<option value="${key}">${cap}</option>`; });
    ...
    function handleListClick(e) {
        ...
        // CORREÇÃO 1: A classe do card é .list-item, não .item-list.
        ...
        if (['add-to-cart-btn', 'edit-price-btn'].some(c => button.classList.contains(c))) {
            ...
            const newPrice = prompt(`Preço de "${item.Item}"?`, price);
            if (newPrice !== null) {

document.addEventListener('DOMContentLoaded', () => {
    ...
    async function postData(payload) {
        ...
        try {
            ...
            if (!payload.action.includes('User')) {
                await fetchData();
            }
        } catch (error) {
    ...
    function createItemCard(item) {
        ...
        const purchasedInfo = ['Comprado', 'Arquivado'].includes(item.Status) ?
    ...
    function populateCategories() {
        ...
        categories.clear();
        fullShoppingList.forEach(i => { if (i.Categoria) categories.add(i.Categoria) });
        dom.categoryFilter.innerHTML = '<option value="all">Todas as Categorias</option>';
        [...categories].sort().forEach(c => { dom.categoryFilter.innerHTML += `<option value="${c}">${c}</option>`; });
    ...
    function populateArchiveFilter() {
        ...
        [...months.entries()].sort().reverse().forEach(([key, value]) => {
            ...
            dom.archiveFilter.innerHTML += `<option value="${key}">${capitalized}</option>`;
        });

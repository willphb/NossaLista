// REGISTRO DO SERVICE WORKER (DEVE ESTAR NO TOPO DO ARQUIVO)
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () => {
    // Usamos ./sw.js para garantir que o caminho seja relativo ao local atual
...
// O RESTO DA APLICAÇÃO COMEÇA AQUI
document.addEventListener('DOMContentLoaded', () => {
    ...
    let vapidPublicKey = null; // Armazenará a chave pública vinda do backend
    ...
    function showApp() {
        ...
        fetchData().then(() => {
            // Só pede permissão para notificação depois que a chave pública foi carregada
    ...
    async function fetchData() {
        ...
        try {
            ...
            vapidPublicKey = responseData.publicKey;
            fullShoppingList = responseData.shoppingList;
            
            if(!Array.isArray(fullShoppingList)) { fullShoppingList = []; throw new Error("Dados da lista inválidos."); }
            ...
            alert("Não foi possível carregar a lista: " + error.message);
        } finally {
            showLoading(false);
        }
    }
    ...
    async function subscribeUserToPush() {
        try {
            ...
            console.error('Falha ao inscrever usuário para notificações:', error);
            if (Notification.permission === 'denied') {
                alert('Você bloqueou as notificações. Para ativá-las, mude as permissões do site nas configurações do seu navegador.');
            }
        }
    }

    function askForNotificationPermission() {
        const isNotificationSupported = 'Notification' in window && 'PushManager' in window && 'serviceWorker' in navigator;
    ...
    function renderLists() {
        ...
        if (!hasPendingOrInCart && !hasPurchased) { dom.pendingList.innerHTML = '<p>Nenhum item encontrado.</p>'; }
    ...
    function renderTotals(list) { let cartTotal = 0, purchasedTotal = 0; list.forEach(i => { const price = parseFloat(i.Preco) || 0; const quantity = parseFloat(i.Quantidade) || 1; if (i.Status === 'No Carrinho') cartTotal += (price * quantity); else if (['Comprado', 'Arquivado'].includes(i.Status)) purchasedTotal += (price * quantity); }); dom.totalsContainer.innerHTML = `<div class="total-box"><h3>Total no Carrinho</h3><p>R$ ${cartTotal.toFixed(2).replace('.', ',')}</p></div><div class="total-box purchased"><h3>Total Gasto (Vista)</h3><p>R$ ${purchasedTotal.toFixed(2).replace('.', ',')}</p></div>`; }
    function createItemCard(item) {
        ...
        else if (item.Status === 'No Carrinho') { priceHtml = `<p class="item-price">R$ ${parseFloat(item.Preco||0).toFixed(2).replace('.',',')}</p>`; actionsHtml = '<button class="edit-price-btn"><i class="fas fa-pencil-alt"></i> Editar Preço</button>'; }
        else if (item.Status === 'Arquivado') { priceHtml = `<p class="item-price">R$ ${parseFloat(item.Preco||0).toFixed(2).replace('.',',')}</p>`; }
    ...
    function handleListClick(e) { const btn = e.target.closest('button'); if (!btn) return; const card = e.target.closest('.list-item'); if (!card) return; const id = card.dataset.id; if (['add-to-cart-btn', 'edit-price-btn'].some(c => btn.classList.contains(c))) { const item = fullShoppingList.find(i => i.ID === id); const price = item && item.Preco > 0 ? item.Preco : ''; const newPrice = prompt(`Preço UNITÁRIO de "${item.Item}"?`, price); if (newPrice !== null) { const n = parseFloat(newPrice.replace(',', '.')); if (!isNaN(n) && n >= 0) { postData({ action: 'moveToCart', id, price: n }); } else { alert('Insira um número válido.'); } } } else if (btn.classList.contains('delete-btn')) { if (confirm('Excluir este item?')) { postData({ action: 'deleteItem', id }); } } }
    ...
    function initializeAppListeners() {
        ...
        if (dom.checkoutBtn) dom.checkoutBtn.addEventListener('click', handleCheckout); if (dom.archiveBtn) dom.archiveBtn.addEventListener('click', handleArchive); if (dom.archiveFilter) dom.archiveFilter.addEventListener('change', renderLists); if (dom.showPurchasedToggle) dom.showPurchasedToggle.addEventListener('change', renderLists); if (dom.categoryFilter) dom.categoryFilter.addEventListener('change', renderLists); if (dom.pendingList) dom.pendingList.addEventListener('click', handleListClick); if (dom.purchasedList) dom.purchasedList.addEventListener('click', handleListClick); if (dom.addItemBtn) dom.addItemBtn.addEventListener('click', () => { dom.addItemForm.reset(); dom.itemQuantity.value = 1; dom.modal.style.display = 'block'; }); if (dom.closeBtn) dom.closeBtn.addEventListener('click', () => dom.modal.style.display = 'none'); if (window) window.addEventListener('click', (e) => { if (e.target == dom.modal) dom.modal.style.display = "none"; }); if (dom.addItemForm) dom.addItemForm.addEventListener('submit', handleFormSubmit);
